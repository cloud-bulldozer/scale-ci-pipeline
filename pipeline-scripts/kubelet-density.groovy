...
def run_kubelet_density = KUBELET_DENSITY.toString().toUpperCase()
def property_file_name = "kubelet-density.properties"

println "Current pipeline job build id is '${pipeline_id}'"

// run uperf
stage ('kubelet-density') {
		if (run_kubelet_density == "TRUE") {
		...
		node(node_label) {
			...
			// get properties file
			sh "wget ${KUBELET_DENSITY_PROPERTIES_FILE} -O ${property_file_name}"
			sh "cat ${property_file_name}"
			def kubelet_density_properties = readProperties file: property_file_name
			def orchestration_user = kubelet_density_properties['ORCHESTRATION_USER']
			def orchestration_host = kubelet_density_properties['ORCHESTRATION_HOST']
			def sshkey_token = kubelet_density_properties['SSHKEY_TOKEN']
			def qps = kubelet_density_properties['QPS']
			def burst = kubelet_density_properties['BURST']
			def node_count = kubelet_density_properties['NODE_COUNT']
			def pods_per_node = kubelet_density_properties['PODS_PER_NODE']
			def es_server = kubelet_density_properties['ES_SERVER']
			def es_port = kubelet_density_properties['ES_PORT']
			def es_index = kubelet_density_properties['ES_INDEX']
			def kubeconfig_file = kubelet_density_properties['KUBECONFIG_FILE']
			def scale_ci_results_token = kubelet_density_properties['SCALE_CI_RESULTS_TOKEN']
			def prom_url = kubelet_density_properties['PROM_URL']
			def metadata_collection = kubelet_density_properties['METADATA_COLLECTION']
			def job_timeout = kubelet_density_properties['JOB_TIMEOUT']
			def step_size = kubelet_density_properties['STEP_SIZE']
			def metrics_profile = kubelet_density_properties['METRICS_PROFILE']
			def log_streaming = kubelet_density_properties['LOG_STREAMING']
			def cleanup = kubelet_density_properties['CLEANUP']
			def cleanup_when_finish = kubelet_density_properties['CLEANUP_WHEN_FINISH']
			def log_level = kubelet_density_properties['LOG_LEVEL']
			...
			try {
				...
				echo "kubelet-density Job failed with the following error: "
				echo "${e.getMessage()}"
				mail(
			...
			println "kubelet-density build ${kubelet_density.getNumber()} completed successfully"
		}
	}
}
