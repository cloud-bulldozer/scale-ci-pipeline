	...
	if (ocp_install == "TRUE") {
		...
		node(node_label) {
			...
			if ( scale_ci_build_trigger.toBoolean() ) {
				sh "curl ${scale_ci_build_trigger_url}/payload -o /tmp/payload.gcp"
				sh "curl ${scale_ci_build_trigger_url}/build.status -o /tmp/status.gcp"
				sh "curl ${scale_ci_build_trigger_url}/cluster.status -o /tmp/cluster_status.gcp"
				cluster_status = readFile "/tmp/cluster_status.gcp"
				status = readFile "/tmp/status.gcp"
				if ( status.toString().trim().equals("PROCEED") || cluster_status.toString().trim().equals("False") ) {
					...
					openshift_install_release_image_override = readFile "/tmp/payload.gcp"
				} else {
			...
			try {
				...
				parameters: [   [$class: 'LabelParameterValue', name: 'node', label: node_label ],
						...
						[$class: 'BooleanParameterValue', name: 'OPENSHIFT_INSTALL_INSTALLER_FROM_SOURCE', value: Boolean.valueOf(openshift_install_installer_from_source) ],
						...
						[$class: 'BooleanParameterValue', name: 'OPENSHIFT_TOGGLE_INFRA_NODE', value: Boolean.valueOf(openshift_toggle_infra_node) ],
						[$class: 'BooleanParameterValue', name: 'OPENSHIFT_TOGGLE_WORKLOAD_NODE', value: Boolean.valueOf(openshift_toggle_workload_node) ],
