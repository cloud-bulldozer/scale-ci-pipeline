	...
	if (ocp_install == "TRUE") {
		...
		node(node_label) {
			...
			if ( scale_ci_build_trigger.toBoolean() ) {
				sh "curl ${scale_ci_build_trigger_url}/payload -o /tmp/payload.aws"
				sh "curl ${scale_ci_build_trigger_url}/build.status -o /tmp/status.aws"
				sh "curl ${scale_ci_build_trigger_url}/cluster.status -o /tmp/cluster_status.aws"
				status = readFile "/tmp/status.aws"
				cluster_status = readFile "/tmp/cluster_status.aws"
				if ( status.toString().trim().equals("PROCEED") || cluster_status.toString().trim().equals("False") ) {
					...
					openshift_install_release_image_override = readFile "/tmp/payload.aws"
				} else {
